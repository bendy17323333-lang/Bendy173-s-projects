...
final class GameViewModel: ObservableObject {
    ...
    func startGame(directive: RunDirective) {
        ...
        bridge.send(
            command: "start",
            payload: directive.payload(briefing: aiDirector.briefingText(for: directive))
        )
    }
    ...
    func restartGame() {
        ...
        bridge.send(
            command: "restart",
            payload: activeDirective.payload(briefing: aiDirector.briefingText(for: activeDirective))
        )
    }
    ...
    func pushSettings() {
        bridge.send(
            command: "settings",
            payload: settings.nativePayload(performance: performance.snapshot)
        )
    }
    ...
    func handleBridgeMessage(_ body: [String: Any]) {
        ...
        case "ready":
            ...
            if let pendingDirective {
                ...
                bridge.send(
                    command: "start",
                    payload: pendingDirective.payload(briefing: aiDirector.briefingText(for: pendingDirective))
                )
            }
        case "state":
