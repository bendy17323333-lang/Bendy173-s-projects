...
struct PhoneGameOverOverlay: View {
            ...
            HStack(alignment: .top, spacing: 8) {
                VStack(alignment: .leading, spacing: 1) {
                    Text(model.activeDirective.title)
                        ...
                        .lineLimit(1)
                    Text("\(model.activeDirective.mode.shortCode) · \(model.activeDirective.shareCode) · ×\(model.activeDirective.scoreMultiplier, specifier: "%.2f")")
                        .font(.system(size: 6, weight: .black, design: .monospaced))
    ...
    private var shareText: String {
        "《零点相位》\(model.activeDirective.mode.title)：\(model.gameOver.score) 分，第 \(model.gameOver.wave) 波，击杀 \(model.gameOver.kills)，风格峰值 \(model.gameOver.peakStyleRank)，擦弹 \(model.gameOver.grazes)。裂隙代码 \(model.activeDirective.shareCode)。"
    }
}
