...
struct NativeCombatFXView: View {
    ...
    private func shieldFX(size: CGSize) -> some View {
        let glowOpacity = 0.26 * (1 - progress) * intensity
        let glowRadius = min(size.width, size.height) * 0.52

        return ZStack {
            RadialGradient(
                colors: [Color.cyan.opacity(glowOpacity), Color.clear],
                center: .center,
                startRadius: 0,
            ...
            ForEach(0..<3, id: \.self) { index in
                ShieldRing(index: index, progress: progress)
            }
        }
    }
...
private struct ShieldRing: View {
    let index: Int
    let progress: Double

    var body: some View {
        let indexValue = Double(index)
        let sideLength = CGFloat(110 + index * 58)
        let strokeOpacity = (1 - progress) * (0.82 - indexValue * 0.18)
        let strokeWidth: CGFloat = index == 0 ? 4 : 1.5
        let ringScale = CGFloat(0.55 + progress * (2.4 + indexValue * 0.36))
        let rotationDirection: Double = index.isMultiple(of: 2) ? 80 : -62
        let rotationDegrees = progress * rotationDirection

        PhaseHexagon()
            .stroke(
                Color.cyan.opacity(strokeOpacity),
