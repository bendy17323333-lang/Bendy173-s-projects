    ...
    private func shieldFX(size: CGSize) -> some View {
        ZStack {
            ...
            ForEach(0..<3, id: \.self) { index in
                PhaseHexagon()
                    .stroke(Color.cyan.opacity((1 - progress) * (0.82 - Double(index) * 0.18)), lineWidth: index == 0 ? 4 : 1.5)
                    .frame(width: 110 + CGFloat(index) * 58, height: 110 + CGFloat(index) * 58)
                    .scaleEffect(CGFloat(0.55 + progress * (2.4 + Double(index) * 0.36)))
                    .rotationEffect(.degrees(progress * (index.isMultiple(of: 2) ? 80 : -62)))
